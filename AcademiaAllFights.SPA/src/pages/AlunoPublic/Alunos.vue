      ...
      no-results-label="Nenhum item corresponde a busca">
        ...
        <template v-slot:top="props">
          ...
          <div v-if="$q.screen.gt.xs" class="col">
            <q-toggle v-model="visibleColumns" val="action" label="Artes Marciais" />
...
export default {
  ...
  data() {
    return {
      ...
      text: "",
      filter: '',
      colunas: [ 'action', 'telefone', 'idade'],
      columns: [
        {name: 'action', label: 'Artes Marciais', align: 'center'},
      ...
      dados: [],
      visibleColumns: ([ 'action', 'nome', 'telefone', 'idade']),
      aluno: {
          nome: '',
          telefone: '',
          idade: '',
  ...
  methods: {
    ...
    getalunos() {
      api.getAlunosAtivo().then(response => {
              if(response.status == 200) {  
