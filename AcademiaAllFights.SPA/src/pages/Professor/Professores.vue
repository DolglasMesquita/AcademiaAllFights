    ...
    <q-card class="area">
      <div class="row">
        <h5 class="titulo text- q-ma-md"> Professores </h5>
          <q-btn to="/professor/adicionar" color="primary" size="sm" class="q-ma-md" icon-right="add"> Adicionar  </q-btn>
      </div>
      <q-table
      :loading="loading"
      title="orgaos"
      ...
      <template v-slot:header="props">
        ...
        <template v-slot:body-cell-action="props">
          <q-td :props="props">
              <q-btn @click="detalhes(props.row.id)" icon="search" color="primary" size="sm" flat dense round/>
              <q-dialog v-model="vizualizar" class="q-pa-md">
                <q-card>
                  <q-card-section>
                    <div class="text-h5"> {{ professor.nome }} </div>
                  </q-card-section>

                  <q-card-section class="q-pt-none">
                    <q-input readonly v-model="professor.cpf" label="Cpf" mask="###.###.###-##" />
                    <q-input readonly v-model="professor.telefone" label="Telefone" mask="(##)#####-####" />
                    <q-input readonly type="date" v-model="professor.dataInicio" label="Data Inicio" />
                    <q-input v-if="!professor.ativo" readonly type="date" v-model="professor.dataFim" label="Data Fim" />
                    <q-input readonly v-model="professor.arteMarcial.nome" label="Arte Marcial" />
                    <br/>
                    <p v-if="professor.ativo"> <strong> Status: </strong> Ativo </p>
                    <p v-if="!professor.ativo"> <strong> Status: </strong> Inativo </p>
                  </q-card-section>
              ...
              <q-btn :to="{path: '/professor/editar/' + props.row.id}" icon="create" color="secondary" size="sm" flat dense round/>
              ...
              <q-card>
                ...
                <q-card-actions align="right">
                  ...
                  <q-btn flat icon-right="delete_forever" label="Remover" color="negative" @click="deletarProfessor()" v-close-popup/>
                </q-card-actions>
              </q-card>
            </q-dialog>
          </q-td>
          ...
          <q-td :props="props" >
            ...
            <q-dialog v-model="confirmDesativar" persistent>
              <q-card>
                ...
                <q-card-actions align="right">
                  <q-btn flat label="Cancelar" color="primary" v-close-popup />
                  <q-btn flat icon-right="toggle_off" label="Desativar" color="negative" @click="desativarProfessor()" v-close-popup />
                </q-card-actions>
              </q-card>
            </q-dialog>
            ...
            <q-dialog v-model="confirmAtivar" persistent>
              <q-card>
                ...
                <q-card-actions align="right">
                  <q-btn flat label="Cancelar" color="primary" v-close-popup/>
                  <q-btn flat icon-right="toggle_on" label="Ativar" color="positive" @click="ativarProfessor()" v-close-popup/>
                </q-card-actions>
              </q-card>
            </q-dialog>
          </q-td>
        ...
        <template v-slot:top="props">
          ...
          <div v-if="$q.screen.gt.xs" class="col">
            ...
            <q-toggle v-model="visibleColumns" val="cpf" label="Arte Marcial" />
...
export default {
  ...
  data() {
    return {
      ...
      text: "",
      filter: '',
      colunas: ['action', 'telefone', 'cpf', 'ativo'],
      columns: [
        ...
        {name: 'cpf', align: 'left', label: 'Arte Marcial', field: 'cpf', sortable: true},
        {name: 'ativo', align: 'center', label: 'Status', align: 'center', field: 'ativo', sortable: true}
      ],
      dados: [],
      visibleColumns: ([ 'action', 'nome', 'telefone', 'cpf', 'ativo' ]),
      professor: {
                nome: '',
                cpf: '',
                telefone: '',
  ...
  methods: {
    getProfessores() {
      api.getProfessores().then(response => {
    ...
    ativarProfessor() {
      api.ativarProfessor(this.idStatus).then(r => {
        this.showNotif(r.data, 'positive');
        this.getProfessores();
      });
    },
    desativarProfessor() {
      api.desativarProfessor(this.idStatus).then(r => {
        this.showNotif(r.data, 'positive');
        this.getProfessores();
      });
    },
    ...
    deletarProfessor() {
      api.removerProfessor(this.idStatus).then(response => {
        if(response.status == 200) {
          this.showNotif('O professor foi removido', 'negative');
          this.getProfessores();
        }
      })
    }
